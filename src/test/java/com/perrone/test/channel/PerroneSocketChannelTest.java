...
public class PerroneSocketChannelTest {

	private static PerroneSocketChannelServer perroneSocketChannelServer;

	@BeforeClass
	public static void init() {
		try {
			perroneSocketChannelServer = new PerroneSocketChannelServer();
			Thread thread = new Thread(perroneSocketChannelServer);
			thread.start();
	...
	public static void destroy() {
		try {
			perroneSocketChannelServer.close();
		} catch (IOException ex) {
	...
	public void socketMatrixTest() {
		PerroneChannelSocket perroneChannelSocket = new PerroneChannelSocket();
		...
		int[][] matrixSum = perroneChannelSocket.connectToServerSocket(Constants.MATRIX_PROCESSOR,
				SocketUtils.convertToBytes(Constants.MATRIX_PROCESSOR, testArray1, testArray1));
		validateMatrixSum(matrixSum, testArray1);
		matrixSum = perroneChannelSocket.connectToServerSocket(Constants.MATRIX_PROCESSOR,
		...
		matrixSum = perroneChannelSocket.connectToServerSocket(Constants.MATRIX_PROCESSOR, null);
		...
		for (int ii = 0; ii < numberOfCalls; ii++) {
			runnables.add(new Callable<int[][]>() {
				...
				public int[][] call() throws Exception {
					PerroneChannelSocket perroneChannelSocket = new PerroneChannelSocket();
					int[][] retValue = perroneChannelSocket.connectToServerSocket(Constants.MATRIX_PROCESSOR,
